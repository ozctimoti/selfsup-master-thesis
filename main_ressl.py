...
from models.densecl_ressl import DenseReSSL
...
def train(train_loader, model, optimizer, epoch, args, rank):
    ...
    loss_glob = AverageMeter('Loss', ':.4e')
    loss_den = AverageMeter('Loss', ':.4e')
    loss_tot = AverageMeter('Loss', ':.4e')

    # top1 = AverageMeter('Acc@1', ':6.2f')
    # top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(len(train_loader), [batch_time, data_time, loss_glob, loss_den, loss_tot],
    ...
    for i, (im_q, im_k) in enumerate(train_loader):
        ...
        with amp.autocast():
            loss_global, loss_dense = model(im_q, im_k)
            loss = loss_global * (1 - args.loss_lambda) + loss_dense * args.loss_lambda
        ...
        with torch.no_grad():
            ...
            loss_glob.update(loss_global.item(), im_q.size(0))
            loss_den.update(loss_dense.item(), im_q.size(0))
            loss_tot.update(loss.item(), im_q.size(0))
