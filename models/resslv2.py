# Only pixel-level ressl
import torch
import torch.nn as nn
import torch.nn.functional as F

class ReSSLv2(nn.Module):
    def __init__(
            self,
            backbone,
            neck,
    ...
    def forward(self, im_q, im_k):
        ...
        logitsq = torch.einsum('nc,ck->nk', [d_q, self.queue.clone().detach()])
        logitsk = torch.einsum('nc,ck->nk', [indexed_k_grid, self.queue.clone().detach()])
