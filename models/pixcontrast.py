...
class PixContrast(nn.Module):
    ...
    def __init__(self, backbone, neck, queue_len=65536, feat_dim=128, momentum=0.999, loss_lambda=0.5, temperature=0.2,
                 tau=0.04, alpha=10):
        super(PixContrast, self).__init__()
    ...
    def forward(self, im_q, im_k, c_q, c_k):
        ...
        l_pos_dense = pixcontrast_sim_q.view(-1).unsqueeze(-1)  # NS^2X1
        ...
        # consistency loss
        Q = torch.exp(l_neg_dense / self.tau)  # NxK

        d_k = d_k.permute(0, 2, 1)  #  N, S^2, C
        d_k = d_k.reshape(-1, d_k.size(2))  # NS^2, C;

        P = torch.exp(torch.einsum('ij,jk->ik', d_k, self.queue2.clone().detach()) / self.tau)

        Q = (Q.transpose(0, 1) / Q.sum(-1)).transpose(0, 1)
        P = (P.transpose(0, 1) / P.sum(-1)).transpose(0, 1)

        # loss_dense_consistency = F.kl_div(P.log(), Q, reduction="batchmean") +
        # F.kl_div(Q.log(), P, reduction="batchmean")
        loss_dense_consistency = self.crit_dense_cons(P.log(), Q) + self.crit_dense_cons(Q.log(), P)
