...
class DenseCL_Consistency(nn.Module):
    def __init__(self, backbone, neck, queue_len=65536, feat_dim=128, momentum=0.999, loss_lambda=0.5, temperature=0.2,
        ...
        # create the first queue for image output
    ...
    def forward(self, im_q, im_k):
        ...
        Qlog = F.log_softmax(l_neg / self.tau, dim=-1)
        Plog = F.log_softmax(torch.einsum('ij,jk->ik', g_k, self.queue.clone().detach()) / self.tau, dim=-1)

        loss_global_consistency = 0.5 * (F.kl_div(Qlog, Plog.detach(), reduction="batchmean", log_target=True) +
                                         F.kl_div(Plog.detach(), Qlog, reduction="batchmean", log_target=True))
        ...
        with torch.no_grad():
            indexed_d_k = indexed_d_k.permute(0, 2, 1)
            indexed_d_k = indexed_d_k.reshape(-1, indexed_d_k.size(2))

        Qlog = F.log_softmax(l_neg / self.tau, dim=-1)
        Plog = F.log_softmax(torch.einsum('ij,jk->ik', indexed_d_k, self.queue2.clone().detach()) / self.tau, dim=-1)

        loss_dense_consistency = 0.5 * (F.kl_div(Qlog, Plog.detach(), reduction="batchmean", log_target=True) +
                                         F.kl_div(Plog.detach(), Qlog, reduction="batchmean", log_target=True))

        self._dequeue_and_enqueue(g_k)
        self._dequeue_and_enqueue2(k2)

        return (loss_global + self.loss_alpha*loss_global_consistency) * (1 - self.loss_lambda) + \
               (loss_dense + self.loss_alpha*loss_dense_consistency) * self.loss_lambda
